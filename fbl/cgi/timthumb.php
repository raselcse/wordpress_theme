...
class timthumb {
	...
	public function __construct(){
		...
		if(BLOCK_EXTERNAL_LEECHERS && array_key_exists('HTTP_REFERER', $_SERVER) && (! preg_match('/^https?:\/\/(?:www\.)?' . $this->myHost . '(?:$|\/)/i', $_SERVER['HTTP_REFERER']))){
			...
			$imgData = devnExt::base64( 'decode', "R0lGODlhUAAMAIAAAP8AAP///yH5BAAHAP8ALAAAAABQAAwAAAJpjI+py+0Po5y0OgAMjjv01YUZ\nOGplhWXfNa6JCLnWkXplrcBmW+spbwvaVr/cDyg7IoFC2KbYVC2NQ5MQ4ZNao9Ynzjl9ScNYpneb\nDULB3RP6JuPuaGfuuV4fumf8PuvqFyhYtjdoeFgAADs=");
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$fp = devnExt::file( 'open', $tempfile,'r',0,$context);
		devnExt::file( 'put', $tempfile4, $this->filePrependSecurityBlock . $imgType . ' ?' . '>'); //6 extra bytes, first 3 being image type
		devnExt::file( 'put', $tempfile4, $fp, FILE_APPEND);
		devnExt::file( 'close', $fp);
		@unlink($tempfile);
		$this->debug(3, "Locking and replacing cache file.");
		$lockFile = $this->cachefile . '.lock';
		$fh = devnExt::file( 'open', $lockFile, 'w');
		if(! $fh){
	...
	public static function curlWrite($h, $d){
		devnExt::file( 'write', self::$curlFH, $d);
	...
	protected function serveCacheFile(){
		...
		$fp = devnExt::file( 'open', $this->cachefile, 'rb');
		if(! $fp){ return $this->error("Could not open cachefile."); }
		fseek($fp, strlen($this->filePrependSecurityBlock), SEEK_SET);
		$imgType = devnExt::file( 'read', $fp, 3);
		fseek($fp, 3, SEEK_CUR);
		...
		$content = devnExt::file( 'get', $this->cachefile);
		if ($content != FALSE) {
	...
	protected function getURL($url, $tempfile){
		...
		if(function_exists('curl_init')){
			...
			self::$curlFH = devnExt::file( 'open', $tempfile, 'w');
			if(! self::$curlFH){
			...
			$curlResult = devnExt::curl( 'exec', $curl);
			devnExt::file( 'close', self::$curlFH);
			...
			if(! devnExt::file( 'put', $tempfile, $img)){
	...
	protected function serveImg($file){
		...
		header ('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
		header ("Pragma: no-cache");
		$bytes = devnExt::file( 'readf', $file);
		if($bytes > 0){
			return true;
		}
		$content = @devnExt::file( 'get', $file);
		if ($content != FALSE){
			echo $content;
			return true;
		}
