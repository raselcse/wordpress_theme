...
if ( ! class_exists('DS_Options') ){
	...
	if(!defined('DS_OPTIONS_DIR')){
		define('DS_OPTIONS_DIR', $fslashed_dir);
	}
	
	if(!defined('DS_OPTIONS_URL')){
		define('DS_OPTIONS_URL', site_url(str_replace( $fslashed_abs, '', $fslashed_dir )));
	}
	
class DS_Options{
	
	protected $framework_url = 'http://devn.co';
	public $framework_version = '1.9.2';
	public $name = 'hoxa';
		
	public $dir = DS_OPTIONS_DIR;
	public $url = DS_OPTIONS_URL;
	public $page = '';
	public $args = array();
	...
	*/
	function __construct($sections = array(), $args = array(), $extra_tabs = array()){
		...
		$defaults['opt_name'] = 'hoxa';//must be defined by theme/plugin
		
		$defaults['google_api_key'] = '';//AIzaSyDAnjptHMLaO8exTHk7i8jYPLzygAE09Hg';//must be defined for use with google webfonts field type
		
		$defaults['menu_icon'] = DS_OPTIONS_URL.'/img/general.png';
		$defaults['menu_title'] = __('Options', 'devn');
		$defaults['page_icon'] = 'icon-themes';
		$defaults['page_title'] = __('Options', 'devn');
		...
		$defaults['footer_credit'] = '<span id="footer-thankyou">&copy; by <a href="'.$this->framework_url.'" target="_blank" title="Visit our site">DEVN</a> Version '.$this->framework_version.'</span>';
		
		$defaults['help_tabs'] = array();
		$defaults['help_sidebar'] = __('', 'devn');
	...
	*/
	function _options_page(){
		...
		if($this->args['page_type'] == 'submenu'){
			...
			$this->page = devn_create_submenu_page(
			...
			$this->page = devn_create_menu_page(
							$this->args['page_title'], 
							$this->args['menu_title'], 
							$this->args['page_cap'], 
							$this->args['page_slug'], 
			...
			devn_create_submenu_page('panel',  
				__('Visual Design Layouts grid - Create sidebars and add widgets','devn'), 
				__('Visual Design Layouts','devn'), 
				'manage_options',  
				'visual-design', 
				'devn_mainPageLayoutGrid'); 
				
			devn_create_submenu_page('panel',  
				__('DEVN Sample Data one-click importer','devn'), 
				__('Sample Data','devn'), 
				'manage_options',  
				'devn-sample-data', 
				'devn_sample_data');
		...
		if(true === $this->args['allow_sub_menu']){
			...
			devn_create_submenu_page($this->args['page_slug'],$this->args['page_title'],'',$this->args['page_cap'],$this->args['page_slug'],create_function( '$a', "return null;" ));
						
						
			foreach($this->sections as $k => $section){
							
				devn_create_submenu_page(
						$this->args['page_slug'],
						$section['title'], 
						$section['title'], 
						$this->args['page_cap'], 
	...
	*/
	function _enqueue(){
		...
		wp_register_style(
			'styleSwitcher', 
			THEME_URI.'/css/color_switcher.css',
			array('farbtastic'),
			'',
			'all'
		);
		...
		wp_localize_script('nhp-opts-js', 'ds_opts', array('reset_confirm' => __('Are you sure? Resetting will loose all custom values.', 'devn'), 'opt_name' => $this->args['opt_name']));
			...
			if(isset($section['fields'])){
				...
				foreach($section['fields'] as $fieldk => $field){
					...
					if(isset($field['type'])){
						...
						$field_class = 'DS_Options_'.$field['type'];
			...
			if(isset($section['fields'])){
				...
				foreach($section['fields'] as $fieldk => $field){
					...
					if(isset($field['validate'])){
						$validate = 'DS_Validation_'.$field['validate'];
		...
		echo '<div class="wrap">';
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="nhp-opts-form-wrapper">';
				...
				echo '<div id="nhp-opts-header">';
					echo '<a target="_blank" href="http://devn.co" class="alignleft"><img src="'.THEME_URI.'/cgi/assets/img/devn.png" /></a>';
					echo '<br /><span style="float:left; margin-left: 20px;"><h3>HOXA by DEVN (ver 1.9.2)</h3></span>';
					echo '<input type="submit" name="" id="" class="btn btn_green" value="'.__('Save Changes','devn').'">';
				...
				echo '</div>';
					...
					if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('nhp-opts-saved') == '1'){
						if(isset($this->options['imported']) && $this->options['imported'] == 1){
							echo '<div id="nhp-opts-imported">'.apply_filters('nhp-opts-imported-text-'.$this->args['opt_name'], __('<strong>Settings Imported!</strong>', 'devn')).'</div>';
						}else{
							echo '<div id="nhp-opts-save">'.apply_filters('nhp-opts-saved-text-'.$this->args['opt_name'], __('<strong>Settings Saved!</strong>', 'devn')).'</div>';
					...
					echo '<div id="nhp-opts-save-warn">'.apply_filters('nhp-opts-changed-text-'.$this->args['opt_name'], __('<strong>Settings have changed!, you should save them!</strong>', 'devn')).'</div>';
					echo '<div id="nhp-opts-field-errors">'.__('<strong><span></span> error(s) were found!</strong>', 'devn').'</div>';
					
					echo '<div id="nhp-opts-field-warnings">'.__('<strong><span></span> warning(s) were found!</strong>', 'devn').'</div>';
					...
					echo '<ul id="nhp-opts-group-menu">';
						...
						if(true === $this->args['show_import_export']){
							echo '<li id="import_export_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="import_export_default"><img src="'.$this->url.'img/glyphicons/glyphicons_082_roundabout.png" /> <span>'.__('Import / Export', 'devn').'</span></a>';
							echo '</li>';
							echo '<li class="divide">&nbsp;</li>';
						}//if
						...
						if(true === $this->args['dev_mode']){
							echo '<li id="dev_mode_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="nhp-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><img src="'.$this->url.'img/glyphicons/glyphicons_195_circle_info.png" /> <span>'.__('Dev Mode Info', 'devn').'</span></a>';
							echo '</li>';
						}//if
				...
				echo '<div id="nhp-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.__('Import / Export Options', 'devn').'</h3>';
							
							echo '<h4>'.__('Import Options', 'devn').'</h4>';
							...
							echo '<div id="nhp-opts-import-code-wrapper">';
								...
								echo '<div class="nhp-opts-section-desc">';
									...
									echo '<p class="description" id="import-code-description">'.apply_filters('nhp-opts-import-file-description',__('Input your backup file below and hit Import to restore your sites options from a backup.', 'devn')).'</p>';
							...
							echo '<div id="nhp-opts-import-link-wrapper">';
								...
								echo '<div class="nhp-opts-section-desc">';
									...
									echo '<p class="description" id="import-link-description">'.apply_filters('nhp-opts-import-link-description',__('Input the URL to another sites options set and hit Import to load the options from that site.', 'devn')).'</p>';
							...
							echo '<p id="nhp-opts-import-action"><input type="submit" id="nhp-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.__('Import', 'devn').'"> <span>'.apply_filters('nhp-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', 'devn')).'</span></p>';
							echo '<div id="import_divide"></div>';
							
							echo '<h4>'.__('Export Options', 'devn').'</h4>';
							echo '<div class="nhp-opts-section-desc">';
					...
					if(true === $this->args['dev_mode']){
						echo '<div id="dev_mode_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.__('Dev Mode Info', 'devn').'</h3>';
							echo '<div class="nhp-opts-section-desc">';
	...
	*/
	function _fonts( $name = '', $index = '' ){
		...
		$fonts = (isset($this->options['googleFonts']))?$this->options['googleFonts']:$this->args['font'];
	...
	function create_css_file(){
		...
		$file = dirname(__FILE__).DS.'..'.DS.'..'.DS.'css'.DS.'options.css';	
		$fp = @devnExt::file( 'open',  $file, 'w');

		$data = '/***'."\n";
		$data .= '*'."\n";
		$data .= '*	copyright (c) devn.co'."\n";
		...
		if( $datas['colorStyle'] != 'none'  ){
			$data .= '@import url(\''.THEME_URI.'/css/colors/'.$datas['colorStyle'].'.css\');'."\n";
		}
		
		$data .= '.limit-width,.layout-boxed,.layout-boxed #header-one-page{max-width:'.($datas['maxwidth']?$datas['maxwidth']:960).'px;}'."\n";
						...
						ob_start();
							...
							if(is_array($devn_option_css_value))
								if( isset($devn_option_css_value['font']) )
									if( $devn_option_css_value['font']!= '' && strpos( $fonts, $devn_option_css_value['font'] ) === false )
							...
							{	
								...
								{								
									...
									devnExt::phpExe('global $devn_option_css_value; $value = $devn_option_css_value; ?>'.$field['css']);
