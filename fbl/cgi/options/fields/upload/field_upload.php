<?php
class DS_Options_upload extends DS_Options{	
	...
	*/
	function render(){
		...
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'">'.__('Browse', 'devn').'</a>';
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'">'.__('Remove Upload', 'devn').'</a>';
	...
	*/
	function enqueue(){
		...
		wp_enqueue_script(
			...
			DS_OPTIONS_URL.'fields/upload/field_upload.js', 
		...
		wp_localize_script('nhp-opts-field-upload-js', 'ds_upload', array('url' => $this->url.'fields/upload/blank.png'));
