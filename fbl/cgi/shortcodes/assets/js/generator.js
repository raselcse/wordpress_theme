jQuery(document).ready(function ($) {
	...
	var $generator = $('#su-generator'),
		...
		$devn_settings = $('#su-generator-settings'),
	...
	$choice.on('click', function (e) {
		...
		$.ajax({
			...
			beforeSend: function () {
				...
				$devn_settings.addClass('su-generator-loading').show();
				// Add narrow class
			...
			success: function (data) {
				...
				$devn_settings.removeClass('su-generator-loading');
				// Insert new HTML
				$devn_settings.html(data);
				...
				$devn_settings.find('.su-generator-attr').on('change keyup blur', function () {
				...
				$.ajax({
					...
					beforeSend: function () {
						// Show loading animation
						// $devn_settings.addClass('su-generator-loading');
					},
					success: function (data) {
						...
						$('.su-generator-toggle-preview').click();
						// Apply selected text to the content field
						if (typeof mce_selection !== 'undefined' && mce_selection !== '') $('#su-generator-content').val(mce_selection);
					},
					dataType: 'json'
	...
	$('#su-generator').on('click', '.su-generator-insert', function (e) {
		...
		var targ = $('#'+window.su_generator_target);
		...
		if( targ.hasClass('replaceOnly') ){
			...
			if( title.get(0) ){
				title.val( $('#su-generator-breadcrumbs span').html() );
			}
		}
		var shortcode = parse();
	...
	$('#su-generator').on('click', '.su-generator-toggle-preview', function (e) {
		...
		$devn_settings.find('input, textarea, select').on('change keyup blur', function () {
			update_preview();
		});
		// Update preview box
		update_preview(true);
	...
	function parse() {
		...
		var query = $selected.val(),
			prefix = $prefix.val(),
			$devn_settings = $('#su-generator-settings .su-generator-attr-container:not(.su-generator-skip) .su-generator-attr'),
	...
	function get() {
		// Prepare data
		var query = $selected.val(),
			$devn_settings = $('#su-generator-settings .su-generator-attr'),
			content = $('#su-generator-content').val(),
			data = {};
		// Add shortcode attributes
		$devn_settings.each(function (i) {
			// Prepare field and value
			var $this = $(this),
				value = '',
				name = $this.attr('name');
	...
	function set(data) {
		// Prepare data
		var $devn_settings = $('#su-generator-settings .su-generator-attr'),
	...
	function update_preview(forced) {
		...
		update_preview_timer = window.setTimeout(function () {
			...
			$preview.html('<h5 id="preview-headding">Preview</h5><br /><iframe scrolling="no" style="width:100%;" onload="su_preview_iframe_onload(this)" src="'+ajaxurl+'?action=su_generator_preview&shortcode='+Base64.encode(shortcode)+'"></iframe>').removeClass('su-generator-loading');
			return;
