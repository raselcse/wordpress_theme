...
class Su_Tools {
	...
	public static function example() {
		...
		// Cache not found
			...
			ob_start();
			// Prepare data
			$code = devnExt::file( 'get',  sanitize_text_field( $_REQUEST['code'] ) );
			// Check for code
			if ( !$code ) die( '<p class="su-examples-error">' . __( 'Example code does not found, please check it later', 'devn' ) . '</p>' );
			// Clean-up the code
			...
			foreach ( $chunks as $chunk ) {
				...
				echo '<div class="su-examples-code"><span class="su-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __( 'Get the code', 'devn' ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
			}
	...
	public static function slide_link_input( $form_fields, $post ) {
		$form_fields['su_slide_link'] = array(
			'label' => __( 'Slide link', 'devn' ),
			'input' => 'text',
			'value' => get_post_meta( $post->ID, 'su_slide_link', true ),
			'helps' => sprintf( '<strong>%s</strong><br>%s', __( 'www.devn.co', 'devn' ), __( 'Use this field to add custom links to slides used with Slider, Carousel and Custom Gallery components', 'devn' ) )
		);
		return $form_fields;
	}
