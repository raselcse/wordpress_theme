...
class Su_Generator {
	...
	public static function button( $args = array() ) {
		...
		$args = wp_parse_args( $args, array(
				'target'    => $target,
				'text'      => __( '<i class=\'icon icon-plus\'></i> Add Component', 'devn' ),
				'class'     => 'button',
				'icon'      => '',//'THEME_URI.'/cgi/shortcodes/assets/images/icon.png',
				'echo'      => true,
				'custom'      => true,
				'title'		=> 'Click to add new component',
				'shortcode' => false
			) );
		// Prepare icon
		if ( $args['icon'] ) $args['icon'] = '<i class="fa fa-' . $args['icon'] . '"></i> ';
		// Print button
		$button = '<a href="javascript:void(0);" class="su-generator-button ' . $args['class'] . '" title="' . $args['title'] . '" data-target="' . $args['target'] . '" data-mfp-src="#su-generator" data-shortcode="' . (string) $args['shortcode'] . '" ' . $args['custom'] . '>' . $args['icon'] . $args['text'] . '</a>';
		...
		<div id="su-generator-wrap" style="display:none">
			<div id="su-generator">
				<div id="su-generator-header">
					<input type="text" name="su_generator_search" id="su-generator-search" value="" placeholder="<?php _e( 'Search for components', 'devn' ); ?>" />
					<div id="su-generator-filter">
						<strong><?php _e( 'Filter by type', 'devn' ); ?></strong>
...
?>
	...
	public static function settings() {
		self::access();
		// Param check
		if ( empty( $_REQUEST['shortcode'] ) ) wp_die( __( 'Shortcode not specified', 'devn' ) );
		// Get cache
		...
		// Cache not found
			...
			$actions = apply_filters( 'su/generator/actions', array(
					'insert' => '<a href="javascript:void(0);" class="button button-primary button-large su-generator-insert"><i class="fa fa-check"></i> ' . __( 'Insert Now', 'devn' ) . '</a>',
					'preview' => '<a href="javascript:void(0);" class="button button-large su-generator-toggle-preview"><i class="fa fa-eye"></i> ' . __( 'Live preview', 'devn' ) . '</a>'
				) );
			// Shortcode header
			$return = '<div id="su-generator-breadcrumbs">';
			$return .= apply_filters( 'su/generator/breadcrumbs', '<a href="javascript:void(0);" class="su-generator-home" title="' . __( 'Click to return to the components list', 'devn' ) . '">' . __( 'Back to all components', 'devn' ) . '</a> &rarr; <span>' . $shortcode['name'] . '</span> <small class="alignright">' . $shortcode['desc'] . '</small><div class="su-generator-clear"></div>' );
			$return .= '</div>';
			// Shortcode note
			...
			if ( count( $shortcode['atts'] ) && $shortcode['atts'] ) {
				...
				foreach ( $shortcode['atts'] as $attr_name => $attr_info ) {
					...
					if ( isset( $attr_info['desc'] ) ) $return .= '<div class="su-generator-attr-desc">' . str_replace( array( '<b%value>', '<b_>' ), '<b class="su-generator-set-value" title="' . __( 'Click to set this value', 'devn' ) . '">', $attr_info['desc'] ) . '</div>';
					$return .= '</div>';
				}
			}
			...
			// Wrapping shortcode
			else $return .= '<div class="su-generator-attr-container"><h5>' . __( 'Content', 'devn' ) . '</h5><textarea name="su-generator-content" id="su-generator-content" rows="5">' . esc_attr( str_replace( array( '%prefix_', '__' ), su_cmpt(), $shortcode['content'] ) ) . '</textarea></div>';
	...
	public static function preview() {
		...
		echo do_shortcode( str_replace( '\"', '"', devnExt::base64( 'decode',  str_replace( ' ' , '+', $_REQUEST['shortcode'])) ) );
	...
	public static function access() {
		if ( !self::access_check() ) wp_die( __( 'Access denied', 'devn' ) );
	}
...
<div class="su-generator-presets alignright" data-shortcode="<?php echo sanitize_key( $_REQUEST['shortcode'] ); ?>">
	<a href="javascript:void(0);" class="button button-large su-gp-button"><i class="fa fa-bars"></i> <?php _e( 'Presets', 'devn' ); ?></a>
	<div class="su-gp-popup">
		<div class="su-gp-head">
			<a href="javascript:void(0);" class="button button-small button-primary su-gp-new"><?php _e( 'Save current settings as preset', 'devn' ); ?></a>
		</div>
	...
	public static function presets_list( $shortcode = false ) {
		...
		if ( is_array( $presets ) && count( $presets ) ) {
			...
			// Hide default text
			echo sprintf( '<b style="display:none">%s</b>', __( 'Presets not found', 'devn' ) );
		}
		// Presets doesn't found
		else echo sprintf( '<b>%s</b>', __( 'Presets not found', 'devn' ) );
	}
	...
	public static function ajax_add_preset() {
		...
		$devn_settings = ( is_array( $_POST['settings'] ) ) ? stripslashes_deep( $_POST['settings'] ) : array();
...
class Shortcodes_Devn_Generator extends Su_Generator {
	function __construct() {
		parent::__construct();
	}
}
