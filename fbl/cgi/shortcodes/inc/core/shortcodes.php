...
class Su_Shortcodes {
	...
	public static function team( $atts = null, $content = null ) {
		...
		$atts = shortcode_atts( array(
				'template'            => 'templates/team-loop.php',
				'id'                  => false,
				'limit_item'      => get_option( 'limit_item' ),
				'post_type'           => 'our-team',
				'taxonomy'            => 'category',
				'column'            	  => get_option( 'column' ),
				'limit_word'          => get_option( 'limit_word' ),
				'category'            => false,
				'order'               => 'DESC',
				'orderby'             => 'date',
				'post_parent'         => false,
		...
		$posts_per_page = intval( $atts['limit_item'] );
		$tag = '';
		$tax_operator = '';
		$tax_term = sanitize_text_field( $atts['category'] );
		$taxonomy = sanitize_key( $atts['taxonomy'] );
		global $devn_teamProw;
		$devn_teamProw = sanitize_key( $atts['column'] );
		global $devn_limit_text;
		$devn_limit_text = sanitize_key( $atts['limit_word'] );
		...
		// Template not found
		else echo '<p class="su-error">Posts: ' . __( 'template not found', 'devn' ) . '</p>';
	...
	public static function work( $atts = null, $content = null ) {
		...
		// Parse attributes
		$atts = shortcode_atts( array(
				'template'            => 'templates/works-loop.php',
				'id'                  => false,
		...
		echo '<script type="text/javascript">document.write(\'<link rel="stylesheet" type="text/css" href="'.THEME_URI.'/css/devn_portfolio.css">\');</script>';
		...
		echo '<script type="text/javascript" src="'.THEME_URI .'/js/devn_portfolio.js"></script>';
	...
	public static function testimonials( $atts = null, $content = null ) {
		...
		// Parse attributes
		$atts = shortcode_atts( array(
				'template'            => 'templates/testimonial-loop.php',
				'id'                  => false,
				'show'     		      => get_option( 'show' ),
				'items'        		  => get_option( 'items' ),
		...
		// Define setting info
		global $devn_limit_word, $devn_display, $devn_rounded, $devn_limit_item, $devn_col;
		$devn_limit_word = sanitize_key( $atts['limit_word'] );
		$devn_limit_item = sanitize_key( $atts['items'] );
		$devn_display = sanitize_key( $atts['show'] );
		$devn_col = sanitize_key( $atts['col'] );
		$devn_rounded = ( bool ) ( $atts['rounded'] === 'yes' ) ? true : false;
		// Set up initial query for post
		$args = array(
			'category_name'  => '',
			'order'          => $order,
	...
	public static function tabs( $atts = null, $content = null ) {
		...
		if ( is_array( self::$tabs ) ) {
			...
			foreach ( self::$tabs as $tab ) {
				...
				$panes[] = '<div class="su-tabs-pane su-clearfix animated fadeIn ' . su_ecssc( $tab ) . '">' . $tab['content'] . '</div>';
			}
	...
	public static function spoiler( $atts = null, $content = null ) {
		...
		return '<div class="su-spoiler su-spoiler-style-' . $atts['style'] . ' su-spoiler-icon-' . $atts['icon'] . su_ecssc( $atts ) . '"' . $atts['anchor'] . '><div class="su-spoiler-title acc-trigger"><a href="#"><span class="su-spoiler-icon"></span>' . su_scattr( $atts['title'] ) . '</a></div><div class="su-spoiler-content su-clearfix acc-container" style="display:none"><div class="content">' . su_do_shortcode( $content, 's' ) . '</div></div></div>';
	}
	...
	public static function box( $atts = null, $content = null ) {
		$atts = shortcode_atts( array(
				'title'       => __( 'This is box title', 'devn' ),
				'style'       => 'default',
				'box_color'   => '#333333',
				'title_color' => '#FFFFFF',
				'color'       => null, // 3.x
	...
	public static function lightbox( $atts = null, $content = null ) {
		...
		if ( !$atts['src'] ) return '<p class="su-error">Lightbox: ' . __( 'please specify correct source', 'devn' ) . '</p>';
	...
	public static function youtube( $atts = null, $content = null ) {
		...
		if ( !$atts['url'] ) return '<p class="su-error">YouTube: ' . __( 'please specify correct url', 'devn' ) . '</p>';
		$atts['url'] = su_scattr( $atts['url'] );
		$id = ( preg_match( '%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $atts['url'], $match ) ) ? $match[1] : false;
		// Check that url is specified
		if ( !$id ) return '<p class="su-error">YouTube: ' . __( 'please specify correct url', 'devn' ) . '</p>';
		// Prepare autoplay
		$autoplay = ( $atts['autoplay'] === 'yes' ) ? '?autoplay=1' : '';
		// Create player
		$return[] = '<div class="su-youtube su-responsive-media-' . $atts['responsive'] . su_ecssc( $atts ) . '">';
		$return[] = '<ifr'.'ame width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="http://www.youtube.com/embed/' . $id . $autoplay . '" frameborder="0" allowfullscreen="true"></ifr'.'ame>';
		$return[] = '</div>';
	...
	public static function youtube_advanced( $atts = null, $content = null ) {
		...
		// Create player
		$return[] = '<div class="su-youtube su-responsive-media-' . $atts['responsive'] . su_ecssc( $atts ) . '">';
		$return[] = '<ifr'.'ame width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="' . $protocol . '://www.youtube.com/embed/' . $id . '?' . $params . '" frameborder="0" allowfullscreen="true"></ifr'.'ame>';
		$return[] = '</div>';
		su_query_asset( 'css', 'su-media-shortcodes' );
		// Return result
		return implode( '', $return );
	...
	public static function vimeo( $atts = null, $content = null ) {
		...
		if ( !$atts['url'] ) return '<p class="su-error">Vimeo: ' . __( 'please specify correct url', 'devn' ) . '</p>';
		$atts['url'] = su_scattr( $atts['url'] );
		$id = ( preg_match( '~(?:<iframe [^>]*src=")?(?:https?:\/\/(?:[\w]+\.)*vimeo\.com(?:[\/\w]*\/videos?)?\/([0-9]+)[^\s]*)"?(?:[^>]*></iframe>)?(?:<p>.*</p>)?~ix', $atts['url'], $match ) ) ? $match[1] : false;
		// Check that url is specified
		if ( !$id ) return '<p class="su-error">Vimeo: ' . __( 'please specify correct url', 'devn' ) . '</p>';
		// Prepare autoplay
		$autoplay = ( $atts['autoplay'] === 'yes' ) ? '&amp;autoplay=1' : '';
		// Create player
		$return[] = '<div class="su-vimeo su-responsive-media-' . $atts['responsive'] . su_ecssc( $atts ) . '">';
		$return[] = '<ifr'.'ame width="' . $atts['width'] . '" height="' . $atts['height'] .
			'" src="http://player.vimeo.com/video/' . $id . '?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff' .
			$autoplay . '" frameborder="0" allowfullscreen="true"></ifr'.'ame>';
		$return[] = '</div>';
		su_query_asset( 'css', 'su-media-shortcodes' );
		// Return result
		return implode( '', $return );
	...
	public static function screenr( $atts = null, $content = null ) {
		...
		if ( !$atts['url'] ) return '<p class="su-error">Screenr: ' . __( 'please specify correct url', 'devn' ) . '</p>';
		$atts['url'] = su_scattr( $atts['url'] );
		$id = ( preg_match( '~(?:<iframe [^>]*src=")?(?:https?:\/\/(?:[\w]+\.)*screenr\.com(?:[\/\w]*\/videos?)?\/([a-zA-Z0-9]+)[^\s]*)"?(?:[^>]*></iframe>)?(?:<p>.*</p>)?~ix', $atts['url'], $match ) ) ? $match[1] : false;
		// Check that url is specified
		if ( !$id ) return '<p class="su-error">Screenr: ' . __( 'please specify correct url', 'devn' ) . '</p>';
		// Create player
		$return[] = '<div class="su-screenr su-responsive-media-' . $atts['responsive'] . su_ecssc( $atts ) . '">';
		$return[] = '<ifr'.'ame width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="http://screenr.com/embed/' . $id . '" frameborder="0" allowfullscreen="true"></ifr'.'ame>';
		$return[] = '</div>';
		su_query_asset( 'css', 'su-media-shortcodes' );
		// Return result
		return implode( '', $return );
	...
	public static function dailymotion( $atts = null, $content = null ) {
		...
		if ( !$atts['url'] ) return '<p class="su-error">Dailymotion: ' . __( 'please specify correct url', 'devn' ) . '</p>';
		$atts['url'] = su_scattr( $atts['url'] );
		$id = strtok( basename( $atts['url'] ), '_' );
		// Check that url is specified
		if ( !$id ) return '<p class="su-error">Dailymotion: ' . __( 'please specify correct url', 'devn' ) . '</p>';
		// Prepare params
		$params = array();
		...
		$return[] = '<ifr'.'ame width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="http://www.dailymotion.com/embed/video/' . $id . '?' . implode( '&', $params ) . '" frameborder="0" allowfullscreen="true"></ifr'.'ame>';
		$return[] = '</div>';
		su_query_asset( 'css', 'su-media-shortcodes' );
		// Return result
		return implode( '', $return );
	...
	public static function audio( $atts = null, $content = null ) {
		...
		if ( !$atts['url'] ) return '<p class="su-error">Audio: ' . __( 'please specify correct url', 'devn' ) . '</p>';
		$atts['url'] = su_scattr( $atts['url'] );
		// Generate unique ID
	...
	public static function video( $atts = null, $content = null ) {
		...
		if ( !$atts['url'] ) return '<p class="su-error">Video: ' . __( 'please specify correct url', 'devn' ) . '</p>';
		$atts['url'] = su_scattr( $atts['url'] );
		// Generate unique ID
	...
	public static function members( $atts = null, $content = null ) {
		$atts = shortcode_atts( array(
				'message'    => __( 'This content is for registered users only. Please %login%.', 'devn' ),
				'color'      => '#ffcc00',
				'style'      => null, // 3.x
				'login_text' => __( 'login', 'devn' ),
	...
	public static function menu_fb() {
		return __( 'This menu doesn\'t exists, or has no elements', 'devn' );
	}
	...
	public static function document( $atts = null, $content = null ) {
		...
		su_query_asset( 'css', 'su-media-shortcodes' );
		return '<div class="su-document su-responsive-media-' . $atts['responsive'] . '"><ifr'.'ame src="http://docs.google.com/viewer?embedded=true&url=' . $atts['url'] . '" width="' . $atts['width'] . '" height="' . $atts['height'] . '" class="su-document' . su_ecssc( $atts ) . '"></ifr'.'ame></div>';
	}
	...
	public static function gmap( $atts = null, $content = null ) {
		...
		su_query_asset( 'css', 'su-media-shortcodes' );
		return '<div class="su-gmap su-responsive-media-' . $atts['responsive'] . su_ecssc( $atts ) . '"><ifr'.'ame width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="http://maps.google.com/maps?q=' . urlencode( su_scattr( $atts['address'] ) ) . '&amp;output=embed"></ifr'.'ame></div>';
	}
	...
	public static function slider( $atts = null, $content = null ) {
		...
		// Slides not found
		else $return = '<p class="su-error">Slider: ' . __( 'images not found', 'devn' ) . '</p>';
		return $return;
	}
	...
	public static function carousel( $atts = null, $content = null ) {
		...
		if ( count( $slides ) ) {
			...
			foreach ( (array) $slides as $slide ) {
				...
				// Slide content with link
				if ( $slide['link'] ) $return .= '<a href="' . $slide['link'] . '"' . $target . '><img src="' . $image. '" alt="' . esc_attr( $slide['title'] ) . '" />' . $title . '</a>';
				// Slide content without link
		...
		// Slides not found
		else $return = '<p class="su-error">Carousel: ' . __( 'images not found', 'devn' ) . '</p>';
		return $return;
	}
	...
	public static function custom_gallery( $atts = null, $content = null ) {
		...
		// Slides not found
		else $return = '<p class="su-error">Custom gallery: ' . __( 'images not found', 'devn' ) . '</p>';
		return $return;
	}
	...
	public static function dummy_text( $atts = null, $content = null ) {
		...
		if ( $return && $atts['cache'] === 'yes' && SU_ENABLE_CACHE ) return $return;
			...
			$devn_xml = simplexml_load_file( 'http://www.lipsum.com/feed/xml?amount=' . $atts['amount'] . '&what=' . $atts['what'] . '&start=0' );
	...
	public static function dummy_image( $atts = null, $content = null ) {
		...
		return '<img src="' . $url . '" alt="' . __( 'Dummy image', 'devn' ) . '" width="' . $atts['width'] . '" height="' . $atts['height'] . '" class="su-dummy-image' . su_ecssc( $atts ) . '" />';
	}
	...
	public static function meta( $atts = null, $content = null ) {
		...
		// Check post ID
		if ( !is_numeric( $atts['post_id'] ) || $atts['post_id'] < 1 ) return sprintf( '<p class="su-error">Meta: %s</p>', __( 'post ID is incorrect', 'devn' ) );
		// Check key name
		if ( !$atts['key'] ) return sprintf( '<p class="su-error">Meta: %s</p>', __( 'please specify meta key name', 'devn' ) );
		// Get the meta
	...
	public static function user( $atts = null, $content = null ) {
		...
		if ( $atts['field'] === 'user_pass' ) return sprintf( '<p class="su-error">User: %s</p>', __( 'password field is not allowed', 'devn' ) );
		// Define current user ID
		if ( !$atts['user_id'] ) $atts['user_id'] = get_current_user_id();
		// Check user ID
		if ( !is_numeric( $atts['user_id'] ) || $atts['user_id'] < 1 ) return sprintf( '<p class="su-error">User: %s</p>', __( 'user ID is incorrect', 'devn' ) );
		// Get user data
	...
	public static function post( $atts = null, $content = null ) {
		...
		if ( !$atts['post_id'] ) $atts['post_id'] = get_the_ID();
		// Check post ID
		if ( !is_numeric( $atts['post_id'] ) || $atts['post_id'] < 1 ) return sprintf( '<p class="su-error">Post: %s</p>', __( 'post ID is incorrect', 'devn' ) );
		// Get the post
	...
	public static function template( $atts = null, $content = null ) {
		...
		// Check template name
		if ( !$atts['name'] ) return sprintf( '<p class="su-error">Template: %s</p>', __( 'please specify template name', 'devn' ) );
		// Get template output
		ob_start();
...
class Shortcodes_Devn_Shortcodes extends Su_Shortcodes {
	function __construct() {
		parent::__construct();
	}
}
