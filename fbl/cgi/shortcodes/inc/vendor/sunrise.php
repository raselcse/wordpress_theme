...
if ( !class_exists( 'Sunrise4' ) ) {
	...
	class Sunrise4 {
		...
		public function register() {
			if ( isset( $this->config['menus'] ) && count( $this->config['menus'] ) )
				foreach ( $this->config['menus'] as $menu ) {
					add_theme_page( $menu['page_title'], $menu['menu_title'], $menu['capability'], $menu['slug'], array( &$this, 'render' ), $menu['icon_url'], $menu['position'] );
				}
			if ( isset( $this->config['pages'] ) && count( $this->config['pages'] ) )
				foreach ( $this->config['pages'] as $page ) {
					add_theme_page( $page['parent_slug'], $page['page_title'], $page['menu_title'], $page['capability'], $page['slug'], array( &$this, 'render' ) );
				}
		}
		...
		public function add_menu( $args ) {
			...
			$args = wp_parse_args( $args, array(
					'page_title'  => __( 'Plugin Settings', 'devn' ),
					'menu_title'  => __( 'Plugin Settings', 'devn' ),
		...
		public function assets() {
			...
			wp_localize_script( 'sunrise', 'sunrise', array(
					'media_title'  => __( 'Choose file', 'devn' ),
					'media_insert' => __( 'Use selected file', 'devn' )
				) );
...
if ( !class_exists( 'Sunrise4_Views' ) ) {
	...
	class Sunrise4_Views {
		...
		public static function closetab( $field, $config ) {
			...
			$return = array();
			$return[] = '</table>';
			if ( $field['actions'] ) $return[] = '<div class="sunrise-actions-bar"><input type="submit" value="' . __( 'Save changes', 'devn' ) . '" class="sunrise-submit button-primary" /><span class="sunrise-spin"><img src="' . admin_url( 'images/wpspin_light.gif' ) . '" alt="" /> ' . __( 'Saving', 'devn' ) . '&hellip;</span><span class="sunrise-success-tip"><img src="' . admin_url( 'images/yes.png' ) . '" alt="" /> ' . __( 'Saved', 'devn' ) . '</span><a href="' . $_SERVER["REQUEST_URI"] . '&amp;sunrise_action=reset" class="sunrise-reset button alignright" title="' . esc_attr( __( 'This action will delete all your settings. Are you sure? This action cannot be undone!', 'devn' ) ) . '">' . __( 'Restore default settings', 'devn' ) . '</a></div>';
			$return[] = '</div>';
		...
		public static function options_page_notices( $options, $config ) {
			...
			$msgs = apply_filters( 'sunrise/page/notices', array(
						__( 'For full functionality of this page it is reccomended to enable javascript.', 'devn' ),
						__( 'Settings saved successfully', 'devn' ),
						__( 'Settings reseted successfully', 'devn' )
					) );
		...
		public static function options_page_panes( $options, $config ) {
			...
			foreach ( $options as $option ) {
				...
				// Show error message
				else $panes[] = call_user_func( array( $config['views_class'], 'notice' ), 'Sunrise: ' . sprintf( __( 'option type %s is not callable', 'devn' ), '<b>' . $option['type'] . '</b>' ), 'error' );
			}
			// Return resulting markup
